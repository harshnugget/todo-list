...
import DragAndDropper from './DragAndDropper';
import { mainLogo } from './images/svg';
import { UITools } from './Manager';
...
const app = () => {
  ...
  const projectDragger = new DragAndDropper(projectList, () => projectManager.projects);

  const taskDragger = new DragAndDropper(taskList, () => projectManager.activeProject.tasks);

  function showDialogElement() {
    const parentElement = UITools.newElement('dialog', {
      class: 'remove-item-dialog',
    });
    const childElements = {
    ...
    // Event listeners
    childElements['yes-btn'].addEventListener('click', () => {
      parentElement.remove();
      localStorage.clear();
      location.reload();
    });

    childElements['no-btn'].addEventListener('click', () => {
      parentElement.remove();
    });
  ...
  function parseLocalStorage() {
    ...
    projectList
      .querySelectorAll('.project-name-input')
